...
//        }
    ...
    GlobalUtils.GetRequest = function (url) {
        var theRequest = new Object(), strs = [], str = "";
        if (url.indexOf("?") != -1) {
            str = url.substr(1);
            strs = str.split("&");
     ...
     * @param content
     */
    GlobalUtils.appendTodDocument = function (chromeUrl, type, content) {
        var custom = document.createElement(type);
        switch (type) {
            case "script":
                if (chromeUrl && !content) {
                    custom.src = chrome.extension.getURL(chromeUrl);
                } else {
            ...
            case "link":
                if (chromeUrl && !content) {
                    ...
                    custom.href = chrome.extension.getURL(chromeUrl);
                } else {
                    custom.textContent = content;
                }
                break;
