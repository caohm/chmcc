...
baidu.endecode = (function () {
    ...
    var a = function (n) {
        n = n.replace(/\\/g, "%").replace("%U", "%u").replace("%u0025", "%25");
...
baidu.ed = (function () {
    ...
    var f = function (i) {
        var h = i.replace(/\\/g, "\\\\").replace(/\\/g, "\\/").replace(/\'/g, "\\'").split("\n");
        var g = h.length;
        var j = [];
    ...
    var e = function () {
        chrome.runtime.onMessage.addListener(function (h, g, i) {
            if (h.type == MSG_TYPE.TAB_CREATED_OR_UPDATED && h.event == "endecode") {
                if (h.content) {
                    document.getElementById("srcText").value = (h.content);
                    d()
                }
            }
        });
