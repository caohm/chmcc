...
$.fn.extend({
    textareaAutoHeight: function (a) {
        this._options = {minHeight: 0, maxHeight: 100000};
        this.init = function () {
            for (var b in a) {
                this._options[b] = a[b]
            }
...
baidu.regexp = (function () {
    ...
    var l = function (u) {
        ...
        t.push('<tr class="active"><th class="num">序号</th><th>匹配结果</th><th>在原字符串中的位置</th></tr>');
        $.each(u, function (v, w) {
            t.push('<tr id="' + n + w.index + '" data-index="' + w.index + '">');
            t.push('<td class="num">' + (v + 1) + '</td><td class="content">' + w.text + '</td><td class="index">' + w.index + "</td>");
            t.push("</tr>")
        });
        t.push("</table>");
        return t.join("")
    ...
    var p = function () {
        $("tr[id^=" + n + "]").click(function (v) {
            ...
            var t = $(b + "[data-id=" + d + u + "]");
            t.animate({opacity: 0}, 200).delay().animate({opacity: 1}, 200).delay().animate({opacity: 0}, 200).delay().animate({opacity: 1}, 200)
        })
    };
    var o = function (u, w) {
        if (!u) {
        ...
        $.each(w, function (x, y) {
            if (x == 0) {
                if (y.index == 0) {
                    ...
                    t.push(c(s, {index: 0, text: u.substring(0, y.index)}));
                    t.push(c(b, y))
                }
            } else {
                v = w[x - 1].index + w[x - 1].text.length;
                t.push(c(s, {index: v, text: u.substring(v, y.index)}));
                t.push(c(b, y))
            }
        });
        j.html(t.join(""));
    ...
    var a = function (t) {
        ...
        u.push('<tr class="active"><th class="num">序号</th><th>匹配结果</th></tr>');
        u.push('<tr><td colspan="2">' + t + "</td></tr>");
        u.push("</table>");
        return u.join("")
    };
    var f = function (w) {
        h.height(q.height() + 24);
        var u = e.val().trim();
        var v = q.val().trim();
        if (!u || !v) {
            r.html(a("不能匹配"));
    ...
    var i = function () {
        $(function () {
            ...
            g = $("#rstCount");
            k = $("#regList");
            e.textareaAutoHeight({minHeight: 34});
            q.textareaAutoHeight({minHeight: 50});
            j.textareaAutoHeight({minHeight: 50});
