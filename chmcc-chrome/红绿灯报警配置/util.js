 ...
 * @returns {*}
 */
function getMainTitle(obj){
    if($(obj).prev(".subkey").length > 0){//二级节点
        if($(obj).parent().parent().find(".text3-monitor-check")[0].checked){
            return $(obj).parent().parent().find(".text3").text();
        }
    }
    return "";
}
...
function getSubTitle(obj){
    if($(obj).prev(".subkey").length > 0){//二级节点
        return $(obj).prev(".subkey").children().text();
    }
    return "";
}
