                ...
                for (; i < len; ++i) {
                    ...
                    if (eqIndex == -1) {
                        ...
                        try {
                            ...
                            throw(e + 'decodeURIComponent error : ' + val, 'error');
                        }
                        if ( endsWith(key, '[]')) {
...
setTimeout(function(){
  ...
  $('a').each(function(index,node){
   ...
   // href && console.log('href:',href.indexOf)
    if(!href || !href.indexOf){
    	//console.log('not href',index);
    	return
    }
    else{
    ...
    //if(href.indexOf('http://item.taobao.com/item.htm?') != -1){
     // var search = $(node).attr('href').replace('http://item.taobao.com/item.htm?','');
      //var opt = unparam(search) || {};
      
      //opt.ali_trackid = '2:mm_32196949_0_0,abc:1358570421_4k6_28767720';
      //delete opt.pid;
      //delete opt.spm; 
     // $(node).attr('href','http://item.taobao.com/item.htm?'+$.param(opt)); 
    //}
    if(href.indexOf('http://ju.mmstat.com') !=-1){
    	$(node).attr('href',href.replace('http://ju.mmstat.com/?url=',''));
    	href = $(node).attr('href');
    }
    if(href.indexOf('http://detail.tmall.com') != -1 || href.indexOf('http://item.taobao.com') != -1){
      //alert('yes')
      var opt = unparam(href.split('?')[1]) ||{};
      var prevHref = href.split('?')[0];
      //opt.pid='mm_32196949_3402274_10983109';
      //if(opt.pid){
        //opt.pid='mm_32196949_3402274_10983109';
      //}
      //else{
      ...
      $(node).attr('href',prevHref+'?'+$.param(opt)+'&ali_trackid=2:mm_32196949_0_0,abc:1358741261_4k7_657072413')
     }
  });
},500);
//#endregion
