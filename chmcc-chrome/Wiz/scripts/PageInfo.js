function Wiz_PageInfo() {
    ...
    var specialCases = {
        ...
        "zhihu.com": "div.zu-main-content",
        "baike.baidu.com": "div.main-wrap",
        "tieba.baidu.com": "div.left_section",
        "renren.com": "div.ugc-detail-container"
    };
    ...
    function findArticle() {
        ...
        if (!hasCheckedArticle) {
            ...
            if (specialCases[host]) {
                candidate = document.querySelector(specialCases[host]);
                if (candidate) {
                    article = candidate;
            ...
            if (!article) {
                ...
                if (urlExtension && (imageTypes.indexOf(urlExtension) != -1)) {
                    candidate = document.querySelector("body > img");
                    if (candidate) {
                        article = candidate;
                        articleBoundingClientRect = article.getBoundingClientRect();
                    }
    ...
    function getSelection() {
        ...
        for (i = 0; i < iframes.length; i++) {
            domain = GlobalUtils.getDomainFromUrl(iframes[i].src);
        ...
        for (i = 0; i < frames.length; i++) {
            domain = GlobalUtils.getDomainFromUrl(frames[i].src);
            if (!domain || domain.lastIndexOf(realDomain) == domain.length - realDomain.length) {
                docs.push(frames[i]);
            }
        }
