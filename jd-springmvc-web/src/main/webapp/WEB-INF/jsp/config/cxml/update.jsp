		...
		<div class="col-sm-10 col-sm-offset-2">
            <div class="container-fluid">
                <ol class="breadcrumb breadcrumb-sm">
                	...
                	<li class="active">修改监控点配置</li>
                </ol>
            </div>
			<div class="container-fluid">
				<form class="form-horizontal" action="/manage/config/cxml/update" method="post">
					<input type="hidden" name="id" value="${scm.id}" />
					<div class="form-group">
						<label class="col-sm-1 control-label">系统名称</label>
						<div class="col-sm-5">
							<input type="text" class="form-control" name="systemName" value="${scm.systemName}" disabled="disabled"/>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">渠道名称</label>
						<div class="col-sm-5">
							<input type="text" class="form-control" name="channelName" value="${scm.channelName}" disabled="disabled"/>
						</div>
					</div>
					<div class="form-group">
						<label class="col-sm-1 control-label">配置文件</label>
						<div class="col-sm-5">
							<textarea rows="20" cols="150" name="configXML" id="configXML">${scm.configXML}</textarea>
						</div>
					</div>
					<div class="form-group">
						<div class="col-sm-offset-1 col-sm-10">
							<button type="submit" class="btn btn-primary" onclick="return updatecfm();">保存</button>
							<button type="button" class="btn btn-default" onclick="history.back()">返回</button>
							<c:if test="${success == 'true'}">
								<span style="color:green" id="tips">修改配置成功！</span>
							</c:if>
							<c:if test="${success == 'fail'}">
								<span style="color:red" id="tips">修改配置失败！</span>
							</c:if>
							<c:if test="${success == 'formatError'}">
								<span style="color:red" id="tips">配置文件格式错误，请验证！</span>
							</c:if>
						</div>
					</div>
				</form>
