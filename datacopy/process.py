...
def execinsert():#调用isql执行delete和insert脚本
    ...
    for i in info_:
        for l in i[2]:
            if '.sql' in l:
               l = l[0:-4]
               r = os.system('/export/home/sybase_iq/OCS-15_0/bin/isql -Udba -Psql -Siq1857000 -Jutf8 -i %s/temp/%s.sql > %s/logs/%s.log & '%(base_dir,l,base_dir,l)) 
               print r
if __name__ =='__main__':
    while 1 :
       x = os.system('/export/home/sybase_iq/OCS-15_0/bin/isql -Udba -Psql -Siq1857000 -Jutf8 -i %s/getstatus.sql > %s/status '%(base_dir,base_dir))
